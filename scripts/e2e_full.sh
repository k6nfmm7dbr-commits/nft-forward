...
R=$(curl -s -X POST "$API/api/rules" -H 'Content-Type: application/json' \
...
curl -s -X PUT "$API/api/rules/$ID/policy" -H 'Content-Type: application/json' \
...
BEFORE_RESET=$(dbq "$ID")
curl -s -X POST "$API/api/rules/$ID/quota/reset" >/dev/null
sleep 4
OUT=$(ip netns exec nff-client1 curl -s --max-time 10 "http://$HOST_IP:$PORT/small")
...
RD=$(curl -s -X POST "$API/api/rules" -H 'Content-Type: application/json' \
...
curl -s -X DELETE "$API/api/rules/$DID" >/dev/null

echo "== 8. SSE 长连接不被 60s 掐断 =="
timeout 70 curl -sN "$API/api/events" >/tmp/nff-sse.out 2>/dev/null &
SP=$!
sleep 68
kill $SP 2>/dev/null
...
echo "== 9. 清理测试规则 =="
curl -s -X DELETE "$API/api/rules/$ID" >/dev/null
